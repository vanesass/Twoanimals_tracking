...
for file in os.listdir(filepath):
  if not file.endswith(".h5"):
   ...
   plot_tracks(video_name,label_loc, selected_label, fileresults) #plot the tracks of both animals for the same selected label
   ...
   for track in [0, 1]:
     ...
     label_vel_tracks[track_name] = smooth_diff(label_loc[:, :, track]) # Extract the velocity variable for the current track
     plot_velocity(label_loc, label_vel_tracks[track_name], video_name, selected_label, track_name, track, fileresults) # Plot using the first plot function
   # 3.2. TRAVELLED DISTANCE - GOAL FOR LAB MEETING
     df_metadata.loc[(df_metadata['Video'] == video_name) & (df_metadata['Track'] == track), 'Travelled distance (pixels)'] = np.sum(label_vel_tracks[track_name], axis=None, dtype=None, out=None)
   # 3.3. AVERAGE VELOCITY
     df_metadata.loc[(df_metadata['Video'] == video_name) & (df_metadata['Track'] == track), 'Avg Velocity (pixels/frame)'] = np.mean(label_vel_tracks[track_name], axis=None, dtype=None, out=None)

...
import os
import pandas as pd
from functions import fill_missing, select_label, smooth_diff, corr_roll, reading_h5files, nameoftracks
...
Nrats = 2 #TODO include in dialog
...
for file in os.listdir(filepath):
  if not file.endswith(".h5"):
   ...
   tracklabel,rowtrack = nameoftracks(df_metadata, video_name, tracklabel, Nrats) #function to name the tracks
   plot_tracks(video_name,label_loc, selected_label,tracklabel, fileresults) #plot the tracks of both animals for the same selected label
   ...
   for track in range(Nrats):
     label_vel_tracks[tracklabel[track]] = smooth_diff(label_loc[:, :, track]) # Extract the velocity variable for the current track
     plot_velocity(label_loc, label_vel_tracks[tracklabel[track]], video_name, selected_label, tracklabel[track], track, fileresults) # Plot using the first plot function
   # 3.2. TRAVELLED DISTANCE - GOAL FOR LAB MEETING
     df_metadata.loc[
         ...
         label_vel_tracks[tracklabel[track]])) * rowtrack["Pixelsize"].values[0]   #covert pixels to cm
     ...
     df_metadata.loc[(df_metadata['Video'] == video_name) & (df_metadata['Track'] == track), 'Avg Velocity (cm/s)'] = (np.mean(   #mean of velocity
         label_vel_tracks[tracklabel[track]]))*rowtrack["Pixelsize"].values[0]*rowtrack["FPS"].values[0] #covert pixels/fr to cm/s
